...
while not control.WantExit:
    ...
    tvel_left = fuzz.trimf(x_tvel, [-5, -3.75, -2.5])
    tvel_right = fuzz.trimf(x_tvel, [2.5, 3.75, 5])
    ...
    tvel_left_level = fuzz.interp_membership(x_tvel, tvel_left, tip_velocity)
    tvel_right_level = fuzz.interp_membership(x_tvel, tvel_right, tip_velocity)

    # Rules
    zero_rule = min(angle_center_level, cvel_zero_level)

    left_rule_1 = min(angle_center_level, tvel_slight_left_level)
    left_rule_2 = min(angle_slight_left_level, tvel_zero_level)
    left_rule = max(left_rule_1, left_rule_2)

    strong_left_rule_1 = min(angle_center_level, tvel_left_level)
    strong_left_rule_2 = min(angle_slight_left_level, tvel_slight_left_level)
    strong_left_rule_3 = min(angle_left_level, tvel_zero_level)
    strong_left_rule = max(strong_left_rule_1, strong_left_rule_2, strong_left_rule_3)
    
    right_rule_1 = min(angle_center_level, tvel_slight_right_level)
    right_rule_2 = min(angle_slight_right_level, tvel_zero_level)
    right_rule = max(right_rule_1, right_rule_2)

    strong_right_rule_1 = min(angle_center_level, tvel_right_level)
    strong_right_rule_2 = min(angle_slight_right_level, tvel_slight_right_level)
    strong_right_rule_3 = min(angle_right_level, tvel_zero_level)
    strong_right_rule = max(strong_right_rule_1, strong_right_rule_2, strong_right_rule_3)
    ...
    # Aggregation
    force_aggregated = np.fmax.reduce([zero_activation, left_activation, strong_left_activation,
                                       right_activation, strong_right_activation])

    # Defuzzification
    force = fuzz.defuzz(x_force, force_aggregated, 'centroid')

    fuzzy_response = force
