...
while not control.WantExit:
    ...
    pos = ctrl.Antecedent(np.linspace(-2.5, 2.5, 41), 'pos')
    pos['left'] = fuzz.trapmf(pos.universe, [-2.5, -2.5, 0, 0])
    pos['center'] = fuzz.trapmf(pos.universe, [-0.3, -0.3, 0.3, 0.3])
    pos['right'] = fuzz.trapmf(pos.universe, [0, 0, 2.5, 2.5])
    ...
    force['slight left'] = fuzz.trimf(force.universe, [-6, -3, 0])
    force['slight right'] = fuzz.trimf(force.universe, [0, 3, 6])
    ...
    rules = [
        ...
        # Left
        ctrl.Rule(angle['center'] & tvel['zero'] & pos['left'] & cvel['zero'], force['slight left']),

        ctrl.Rule(angle['slight left'] & tvel['zero'], force['left']),
        
        ctrl.Rule(angle['slight left'] & tvel['slight left'] |
                  angle['left'] & tvel['zero'], force['strong left']),

        # Right
        ctrl.Rule(angle['center'] & tvel['zero'] & pos['right'] & cvel['zero'], force['slight right']),

        ctrl.Rule(angle['slight right'] & tvel['zero'], force['right']),

        ctrl.Rule(angle['slight right'] & tvel['slight right'] |
    ...
    def put(name, value):
        try:
            fuzz_simulation.input[name] = value
        except:
            pass
    fuzz_simulation.put = put

    fuzz_simulation.put('pos', cart_position)
    fuzz_simulation.put('cvel', cart_velocity)
    fuzz_simulation.put('angle', pole_angle)
    fuzz_simulation.put('tvel', tip_velocity)
